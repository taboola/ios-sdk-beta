...
/*! @brief Optional. The identifier for the user in the publisher’s eco-system. */
 ...
 */
- (TBLStoriesUnit*)createStoryUnitWithPlacementName:(NSString *)placementName mode:(NSString *)mode delegate:(id<TBLStoriesUnitDelegate>)delegate;
...
/// Refresh content in all units of this TBLClassicPage.
- (void)refreshPage __deprecated_msg("The method is deprecated. Please use fetchAllUnitsContent for refreshing");

/*!
 Refresh content in all units of this TBLClassicPage using updateOnly.
 */
- (void) updateContentPageOnly;

/*!
 Fetches content on all it's TBLClassicUnit objects using with fetchOnly.
 */
- (void) fetchAllUnitsContentOnly;

/*!
  Clears any pending fetch content requests that may be stuck
 */
- (void)clearAllFetchRequests;
